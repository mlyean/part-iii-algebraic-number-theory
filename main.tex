...
\begin{theorem}[Chebotarev Density Theorem]\label{thm:8_13}
    Let $L/K$ be Galois, $G = \Gal(L/K)$. Fix $\sigma \in G$, and suppose that $\sigma$ has $c$ conjugates in $G$. Then the set of primes $\fp$ of $K$ which has a prime $\cp$ in $L$ lying above it such that $\leg{L/K}{\fp} = \sigma$ has density $\frac{c}{\abs{G}}$.
\end{theorem}
