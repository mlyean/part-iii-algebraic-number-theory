...
\begin{definition}[Gauss Sum]\label{def:7_21}
    Let $\zeta$ be a primitive $m$\textsuperscript{th} root of unity. Then for $k \ge 0$, we define the Gauss sum $\tau_K(\chi) = \sum_{a \in (\ZZ / m\ZZ)^\times} \chi(a) \zeta^{ak}$.
\end{definition}
...
\begin{alignat*}{3}
    ...
    &= \begin{cases}
        \displaystyle-\frac{2 \tau_1(\chi)}{m} \sum_{k \in (\ZZ / m\ZZ)^\times} \overline{\chi}(k) \log \sin \frac{k \pi}{m} & \text{if } \chi \text{ is even}\\ %TODO: Where does the 2 come from?
...
\begin{theorem}\label{thm:7_26}
    Let $\chi$ be a primitive character ad $m \ge 3$. Then
    \begin{equation*}
...
Let $K = \QQ(\sqrt{d})$, $d$ squarefree. Let $m = \abs{d_K}$. Then $K \subseteq \QQ(\zeta_m)$. By \autoref{thm:7_26}, we have $\rho = L(\chi, 1)$, where $\chi$ is the unique non-trivial character mod $m$ which corresponds to a character of $\Gal(K/\QQ)$. For $p \nmid m$,
\begin{equation*}
    \chi(p) = \begin{cases}
        1 & \text{if } p \text{ splits}\\
        -1 & \text{if } p \text{ is inert}
