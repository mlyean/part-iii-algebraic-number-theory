...
Let $K$ be an abelian extension of $\QQ$. Then $K \subseteq \QQ(\zeta_m)$ for some $m$. Identify $\Gal(\QQ(\zeta_m) / \QQ)$ with $(\ZZ / m\ZZ)^\times$, and let $G = \Gal(K / \QQ)$. We consider $\widehat{G}$ as a subgroup\footnote{The exact sequence $(\ZZ / m \ZZ)^\times \to G \to 0$ gives an exact sequence $0 \to \widehat{G} \to \widehat{(\ZZ / m \ZZ)^\times}$} of $\widehat{(\ZZ / m\ZZ)^\times}$. Then we adapt the proof of \autoref{prop:7_17} to find for $p \nmid m$,
\begin{equation*}
...
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
We define it for all $(m, n) = 1$ by extending multiplicatively. It turns out that $\chi$ is primitive mod $m$, it is even if $d > 0$ and odd if $d < 0$. % TODO: Why?
...
\begin{proof}[Proof of \autoref{prop:3_10}]
    Since $\ff(L/K) \mid \fm$, we know $P_K(\fm) \subseteq \ker \Phi_\fm \subseteq I_K(\fm)$. For $\fa \in I_K(\fm)$, $\leg{L/K}{\fa}$ depends only on the class of $\fa$ modulo $P_K(\fm)$. By the Chebotarev density theorem, there are infinitely many primes $\fp$ in $\cO_K$ such that $\leg{K_\fm/K}{\fp} = \leg{K_\fm/K}{\fa}$. Choose such a prime $\fp$ with $\fp \nmid \fm$. Since we have an isomorphism $I_K(\fm) / P_K(\fm) \cong \Gal(K_\fm / K)$, this implies that $\fa$ is equivalent to a prime $\fp$ modulo $P_K(\fm)$.

    Now suppose $\fp \subseteq \cO_K$ is such that $\fp \in \ker(\Phi_\fm)$. Then $\leg{L/K}{\fp} = 1$, so $\fp$ splits completely in $L$. Thus if $\cp \subseteq \cO_L$ such that $\cp \mid \fp$, then $\fp = N_{L/K}(\cp) \in N_{L/K}(I_L(\fm)) \subseteq N_{L/K}(I_L(\fm)) P_K(\fm)$.
\end{proof}
...
\marginpar{\fbox{Lecture 23}}
    Recall we had a map $(\cdot) : \II_K \to I_K$, $\alpha \mapsto (\alpha) = \prod_{\fp \nmid \infty} \fp^{v_\fp(\alpha_\fp)}$, and an isomorphism $\II_K / \II_K(\fm)K^\times \cong C_K/C_K(\fm)$.

    Let $\II_K^{\langle \fm \rangle} = \{\alpha \in \II_K \mid \alpha_\fp = 1 \text{ for } \fp \mid \fm\}$, which is a subgroup of $\II_K$. We claim that $\II_K = \II_K^{\langle \fm \rangle} \II_K(\fm) K^\times$. Let $\alpha \in \II_K$. Write $\fm = \prod_{\fp} \fp^{n_\fp}$. By the approximation theorem, there is an $x \in K^\times$ such that $\alpha_\fp x \equiv 1 \pmod{\fp^{n_\fp}}$ for $\fp \mid \fm$. Write $\alpha x = \alpha' \beta$, where
    \begin{equation*}
    ...
    \end{align*}
    The homomorphism $\II_K^{\langle \fm \rangle} \to I_K(\fm)/P_K(\fm)$ given by $\alpha \mapsto (\alpha) \pmod{P_K(\fm)}$  is surjective with kernel $\II_K(\fm) K^\times \cap \II_K^{\langle \fm \rangle}$. This induces an isomorphism $\II_K^{\langle \fm \rangle} / \II_K(\fm) K^\times \cap \II_K^{\langle \fm \rangle} \cong I_K(\fm)/P_K(\fm)$ as required. %TODO: Check
\end{proof}
...
Let $K$ be a local field. Let $K^\sep$ be a fixed separable closure. Then write $K^\ab$ for the maximal abelian extension of $K$ inside $K^\sep$, i.e. $K^\ab = \bigcup_{\substack{L/K \text{ finite abelian} \\ L \subseteq K^\sep}} L$.
...
A \emph{norm group} of a local field $K$ is a subgroup of $K^\times$ of the form $N_{L/K}(L^\times)$ for some $L/K$ finite abelian.
...
\begin{corollary}\label{cor:11_5}
    ...
    \end{equation*}
    For a principal idele $a \in K^\times$, we have the product formula
    \begin{equation*}
        \prod_\fp (a, L_\cp / K_\fp) = 1
    \end{equation*}
\end{corollary}
Note this implies if $\pi$ is a uniformiser of $K_\fp$, then $(\overline{n}_\fp(\pi), L/K) = (\pi, L_\cp / K_\fp) = \Frob_{L_\cp / K_\fp} = \leg{L/K}{\fp}$ (recalling that $D_{\cp/\fp} = \Gal(L_\cp / K_\fp)$). This then finishes the proof of \autoref{thm:10_4}.
...
\begin{proposition}\label{prop:11_6}
    Let $L/K$ be a finite abelian extension of number field, then
    \begin{equation*}
        \ff(L/K) = \prod_{\cp \mid \fp} \ff(L_\cp / K_\fp)
    \end{equation*}
\end{proposition}
