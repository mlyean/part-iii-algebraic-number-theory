...
\begin{example}\label{eg:2_11}
    ...
    {\color{blue}
        For a prime $p \nmid m$, let $\cp$ be a prime lying over $(p)$. Let $\sigma = \leg{L/K}{\cp}$. Since the polynomial $x^m-1$ is separable modulo $\cp$, the $m$\textsuperscript{th} roots of unity $\zeta_m^k$ for $0 \le k \le m-1$ are all distinct modulo $\cp$. By definition, $\sigma(\zeta_m) \equiv \zeta_m^p \pmod{\cp}$. Thus $\sigma(\zeta_m) = \zeta_m^p$, so $\Phi((p)) = [p] \in (\ZZ / m \ZZ)^\times$.
    }
...
\begin{definition}\label{def:9_16}
    ...
    \begin{equation*}
        C_K(\fm) = I_K(\fm) K^\times / K^\times \subseteq C_K
    \end{equation*}
